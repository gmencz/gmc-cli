import {
  ...
  sumTestContent,
  tsconfigNode,
} from '../generated-files-content'

test('.gitignore has the expected config', () => {
  expect(gitIgnoreContent).toMatchInlineSnapshot(`
    "# Logs
    logs
    *.log
    npm-debug.log*
...
test('.prettierrc has the expected config', () => {
  expect(prettierrcContent).toMatchInlineSnapshot(`
    "{
      \\"arrowParens\\": \\"avoid\\",
      \\"bracketSpacing\\": true,
...
test('.huskyrc.js has the expected config', () => {
  expect(huskyrcContent).toMatchInlineSnapshot(`
    "module.exports = {
      hooks: {
...
test('.lintstagedrc.js has the expected config', () => {
  expect(lintstagedrcContent).toMatchInlineSnapshot(`
    "module.exports = {
...
test('.eslintrc.js has the expected config', () => {
  expect(eslintNodeJestContent).toMatchInlineSnapshot(`
    "module.exports = {
      env: {
        es2021: true,
        node: true,
...
test('jest has the expected config', () => {
...
test('tsconfig.json has the expected config', () => {
  expect(tsconfigNode).toMatchInlineSnapshot(`
    "{
      \\"compilerOptions\\": {
        \\"target\\": \\"ESNext\\",
        \\"module\\": \\"commonjs\\",
...
test('example index source file has the expected source', () => {
  expect(indexContent).toMatchInlineSnapshot(
    `"export const sum = (a: number, b: number): number => a + b"`,
  )
})

test('example sum test file has the expected source', () => {
  expect(sumTestContent).toMatchInlineSnapshot(`
...
test('README has the expected contnet', () => {
  expect(readmeContent('test-project')).toMatchInlineSnapshot(`
    "# test-project
